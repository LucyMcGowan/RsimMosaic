composeMosaicFromImageRandomOptim <-
  function(originalImageFileName,
           outputImageFileName,
           imagesToUseInMosaic,
           inputTileSize,
           useGradients = FALSE,
           removeTiles = TRUE,
           fracLibSizeThreshold = 0.7,
           repFracSize = 0.25,
           verbose = TRUE,
           neig = 20) {
    #
    ...
    filenameArray <-
      list.files(imagesToUseInMosaic, full.names = TRUE)
    ##TODO
    xTileSize <- inputTileSize
    yTileSize <- inputTileSize
    if (verbose) {
    ...
    if (verbose) {
      ...
      cat(paste(
        ...
        ((xOrigImgSize - 2) * xTileSize),
        " x ",
        ((yOrigImgSize - 2) * yTileSize),
        "\n"
      ))
    }
    
    libForMosaicFull <-
      libForMosaicFull[stats::complete.cases(libForMosaicFull), ]
    # Create the mapping functions that will allow the usage of most of the library tiles
    rInterp <-
      approxfun(
        x = summary(as.vector(originalImage[, , 1])),
        y = summary(libForMosaicFull[, 2]),
        rule = 2
      )
    gInterp <-
      approxfun(
        x = summary(as.vector(originalImage[, , 2])),
        y = summary(libForMosaicFull[, 3]),
        rule = 2
      )
    bInterp <-
      approxfun(
        x = summary(as.vector(originalImage[, , 3])),
        y = summary(libForMosaicFull[, 4]),
        rule = 2
      )
    ...
    outputImage <-
      array(dim = c(((xOrigImgSize - 2) * xTileSize), ((yOrigImgSize - 2) * yTileSize), 3))
    removedList <- c()
    
    # create the list of pixel coordinates in the original image
    l <- 1
    pCoord <-
      matrix(nrow = ((xOrigImgSize - 2) * (yOrigImgSize - 2)), ncol = 2)
    ...
    for (i in 1:npixels) {
      ...
      pixelRGBandNeigArray <-
        computeStatisticalQuantitiesPixel(pCoord[idx, 1], pCoord[idx, 2], originalImage, useGradients)
      
      pixelRGBandNeigArray <-
        c(
          rInterp(pixelRGBandNeigArray[1]),
          gInterp(pixelRGBandNeigArray[2]),
          bInterp(pixelRGBandNeigArray[3])
        )
      
      # get the closest match from the library
      tileFilename <-
        getCloseMatch(pixelRGBandNeigArray, libForMosaic, nneig = neig)
      ...
      if (removeTiles) {
        ...
        if (length(libForMosaic[, 1]) < (fracLibSizeThreshold * length(libForMosaicFull[, 1]))) {
          idxs <-
            runif(round(0.25 * length(libForMosaicFull[, 1])), 1, length(removedList))
          for (ii in 1:length(idxs)) {
            libForMosaic <-
              addBackTile(removedList[idxs[ii]], libForMosaic, libForMosaicFull)
          }
